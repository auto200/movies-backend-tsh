...
describe("MoviesService", () => {
  ...
  test("get movies with duration filter", () => {
    ...
    const expected1 = [initialData.movies[1]];
    expect(moviesService.getMoviesWithFilters({ duration: 90 })).resolves.toEqual(expected1);

    const expected2 = [initialData.movies[0]];
    expect(moviesService.getMoviesWithFilters({ duration: 49 })).resolves.toEqual(expected2);
  });
});
